 ...
 */
global $nrgbusiness_social_icons;
$nrgbusiness_social_icons = array(
    "facebook" => "facebook",
    "twitter" => "twitter",
    "pinterest" => "pinterest",
    "instagram" => "instagram",
...
function admin_common_render_scripts() {
    ...
    wp_enqueue_style('themeton-admin-common-style', nrgbusiness_file_require(get_template_directory_uri().'/framework/admin-assets/common.css', true) );

    wp_enqueue_script('jquery');
    wp_enqueue_script('wp-color-picker');
    
    wp_enqueue_script('themeton-admin-common-js', nrgbusiness_file_require(get_template_directory_uri().'/framework/admin-assets/common.js', true), array('jquery'), false, true);
}
...
function comment_count_text() {
    ...
    $comment_text = $comment_count . ' ' . __('Comments', 'nrgbusiness');
    if( (int)$comment_count == 1 ){
        $comment_text = $comment_count . ' ' . __('Comment', 'nrgbusiness');
    }
    else if( (int)$comment_count < 1 ){
        $comment_text = __('No Comment', 'nrgbusiness');
    }
...
if (!function_exists('tt_comment_form')) :
    ...
    function tt_comment_form($fields) {
        ...
        $fields = array(
            'author' => '<p class="comment-form-author">' . '<label for="author">' . __('Name', 'nrgbusiness') . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
            '<input placeholder="' . __('Name', 'nrgbusiness') . '" id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . $aria_req . ' /></p>',
            'email' => '<p class="comment-form-email"><label for="email">' . __('Email', 'nrgbusiness') . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
            '<input placeholder="' . __('Email', 'nrgbusiness') . '" id="email" name="email" type="text" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . $aria_req . ' /></p>',
            'url' => '<p class="comment-form-url"><label for="url">' . __('Website', 'nrgbusiness') . '</label>' .
            '<input placeholder="' . __('Website', 'nrgbusiness') . '" id="url" name="url" type="text" value="' . esc_attr($commenter['comment_author_url']) . '" size="30" /></p>',
        );
        return $fields;
    }
...
if (!function_exists('about_author')) {
    ...
    function about_author() {
        ...
        <div class="item-author clearfix">
            ...
            ?>
            <h3><?php esc_attr_e('Written by ', 'nrgbusiness'); ?><?php if (is_author()) the_author(); else the_author_posts_link(); ?></h3>
            ...
            <p>
                ...
                if ($description != '')
                    ...
                    esc_attr_e('The author didnt add any Information to his profile yet', 'nrgbusiness');
                ?>
            </p>
        </div>
        <?php
...
if (!function_exists('social_share')) {
     ...
     * @global array $nrgbusiness_social_icons
     * @global type $post : Current post
     */
    function social_share() {
        global $nrgbusiness_social_icons, $post;
        
        echo '<span class="sf_text">' . __('Share', 'nrgbusiness') . ': </span>';
        echo '<ul class="post_share list-inline">';
        if( TT::get_mod('share_facebook')=='1' ) {
            echo '<li><a href="https://www.facebook.com/sharer/sharer.php?u=' . get_permalink() . '" title="Facebook" target="_blank"><i class="fa ' . $nrgbusiness_social_icons['facebook'] . '"></i></a></li>';
        }
        if( TT::get_mod('share_twitter')=='1' ) {
            echo '<li><a href="https://twitter.com/share?url=' . get_permalink() . '" title="Twitter" target="_blank"><i class="fa ' . $nrgbusiness_social_icons['twitter'] . '"></i></a></li>';
        }
        if( TT::get_mod('share_google_plus')=='1' ) {
            echo '<li><a href="https://plus.google.com/share?url='.get_permalink().'" title="GooglePlus" target="_blank"><i class="fa ' . $nrgbusiness_social_icons['googleplus'] . '"></i></a></li>';
        }
        if( TT::get_mod('share_pinterest')=='1' ) {
            $image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'large');
            echo '<li><a href="//pinterest.com/pin/create/button/?url=' . get_permalink() . '&media=' . $image[0] . '&description=' . get_the_title() . '" title="Pinterest" target="_blank"><i class="fa ' . $nrgbusiness_social_icons['pinterest'] . '"></i></a></li>';
        }
        if( TT::get_mod('share_email')=='1' ) {
            echo '<li><a href="mailto:?subject=' . strip_tags(get_the_title()) . '&body=' . strip_tags(get_the_excerpt()) . get_permalink() . '" title="Email" target="_blank"><i class="fa ' . $nrgbusiness_social_icons['email'] . '"></i></a></li>';
        }
        echo '</ul>';
...
if (!function_exists('tt_admin_bar_menu')) {
    ...
    function tt_admin_bar_menu() {
        ...
        $customizer = array(
            ...
            'title' => __('Site Customize', 'nrgbusiness'),
        ...
        $customizer = array(
            ...
            'title' => __('Import Demo', 'nrgbusiness'),

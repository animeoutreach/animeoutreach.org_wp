...
class NRGbusiness_ImportData{
    ...
    public function demo_import_page(){
        ...
        $templates = '';
        $file_dir = nrgbusiness_file_require( get_stylesheet_directory().'/framework/demo-data/data/' );
        foreach( glob( $file_dir . '*.xml' ) as $filename ) {
            ...
            $img = nrgbusiness_file_require( trailingslashit(get_template_directory_uri()).'framework/demo-data/thumbs/'.$img.'.png', true );
            $templates .= '<label>
    ...
    public function import_data($param){
        ...
        /* File Paths */
        $options_file = "/framework/demo-data/customizer.txt";

        $nav_file = nrgbusiness_file_require(get_template_directory_uri().'/framework/demo-data/navigation.xml', true);
        $nav_file_path = nrgbusiness_file_require(get_template_directory().'/framework/demo-data/navigation.xml');

        $widget_file = nrgbusiness_file_require(get_template_directory_uri().'/framework/demo-data/widgets.txt', true);
        $widget_file_path = nrgbusiness_file_require(get_template_directory().'/framework/demo-data/widgets.txt');


        if( isset($param['template']) && !empty($param['template']) ){

            /* Import Selected Content */
            $data_file = nrgbusiness_file_require(get_template_directory().'/framework/demo-data/data/'.$param['template'].'.xml');
            ...
            if( isset($param['import_theme_options']) && $param['import_theme_options']=='1' ){
                $page_opt = substr($param['template'], 0, 2);
                $page_opt_path = get_template_directory().'/framework/demo-data/page_options/'.$page_opt.'.txt';
                $page_opt_uri = get_template_directory_uri().'/framework/demo-data/page_options/'.$page_opt.'.txt';
                if( file_exists($page_opt_path) ){
                    $get_page_opt_file = wp_remote_get($page_opt_uri);
                    $page_opt_body = array_key_exists('body', $get_page_opt_file) ? $get_page_opt_file['body'] : '';
                    if( !empty($page_opt_body) ){
                        $customizer = new TT_Theme_Customizer();
                        $customizer->wp_customizer_import_handler($page_opt_body);
                    }
                }
            }
            /*=== End Contents */
            ...
            /* Theme Options */
            if( isset($param['import_theme_options']) && $param['import_theme_options']=='1' ){
                if( file_exists( nrgbusiness_file_require(get_template_directory().$options_file) ) ){
                    $options_uri = nrgbusiness_file_require( get_template_directory_uri().$options_file, true );
            ...
            try{
                $main_file = nrgbusiness_file_require(get_template_directory().'/framework/demo-data/main-content.xml');
    ...
    */
    public function import_revo(){
        if( class_exists('UniteFunctionsRev') ){
            ...
            $revo_directory = get_template_directory().'/framework/demo-data/revsliders/';
            $revo_files = array();
            $db = new UniteDBRev();
            ...
            foreach( glob( $revo_directory . '*.txt' ) as $filename ) {
                $filename = basename($filename);
                $revo_files[] = get_template_directory_uri().'/framework/demo-data/revsliders/'.$filename;
            }

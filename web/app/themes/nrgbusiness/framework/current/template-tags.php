...
class TPL{
    ...
    public static function pagination( $query=null ) {
        ...
        echo '<ul class="pagination">';
            echo '<li>' . esc_attr__('Pages', 'nrgbusiness') . '</li>';
                ...
                if($post->post_type == 'portfolio'){
                    echo '<div class="row m0 youMightLike '.$post_type_class.'">
                            <div class="row sectionTitle">
                                <h4>' . esc_attr__('Related Projects', 'nrgbusiness') . '</h4>
                            </div>
                            <div class="row">
                                '. $html .'
                            </div>
                    ...
                    echo '<div class="row m0 youMightLike '.$post_type_class.'">
                            <h4>' . esc_attr__('You might also like', 'nrgbusiness') . '</h4>
                            <div class="row">
                                '. $html .'
                            </div>
                          </div>';
            ...
            <div class="container">
                <form role="search" method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
                    <div class="input-group">
                        <input type="search" class="search-field" placeholder="<?php esc_attr_e('Type and hit Enter ...', 'nrgbusiness'); ?>" value="" name="s" autocomplete="off">
                        <input type="submit" class="search-submit" value="<?php esc_attr_e('Go', 'nrgbusiness'); ?>">
...
function get_post_comment_count(){
    ...
    if( $comments==0 )
        $comments = esc_attr__('No Comments', 'nrgbusiness');
    elseif( $comments==1 )
        $comments = esc_attr__('1 Comment', 'nrgbusiness');
    else
        $comments = $comments . esc_attr__(' Comments', 'nrgbusiness');
...
// Comment Navigation
if ( ! function_exists( 'nrgbusiness_comment_nav' ) ) :
    function nrgbusiness_comment_nav() {
        // Are there comments to navigate through?
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
        ?>
        <nav class="navigation comment-navigation" role="navigation">
            <h2 class="screen-reader-text"><?php esc_attr_e('Comment navigation', 'nrgbusiness'); ?></h2>
            <div class="nav-links">
                <?php
                    if ( $prev_link = get_previous_comments_link( esc_attr__('Older Comments', 'nrgbusiness') ) ) :
                        printf( '<div class="nav-previous">%s</div>', $prev_link );
                    endif;

                    if ( $next_link = get_next_comments_link( esc_attr__('Newer Comments', 'nrgbusiness') ) ) :
...
function get_tags_list(){
    global $post;
    $tags_list = get_the_tag_list( '', _x( ' ', 'Used between list items, there is a space after the comma.', 'nrgbusiness' ) );
    if ( $tags_list ) {
        printf( '<div class="tag-links text-right"><span class="sf_text hidden-xxs">%1$s: </span>%2$s</div>',
            _x( 'Tags', 'Used before tag names.', 'nrgbusiness' ),
            $tags_list
        );
    }
}
...
function get_share_links(){
    global $post;
    echo '<span class="sf_text hidden-xxs">'. esc_attr__('Share on:', 'nrgbusiness') .' </span>';
...
function nrgbusiness_custom_comment($comment, $args, $depth) {
    ...
    <<?php echo esc_attr($tag); ?> class="post pingback">
        <p><?php esc_attr_e('Pingback:', 'nrgbusiness'); ?> <?php comment_author_link(); ?><?php edit_comment_link( esc_attr__('Edit', 'nrgbusiness'), '<span class="edit-link">', '</span>' ); ?></p>
    <?php
            break;
        default:
    ?>
    ...
    <div class="comment-box">
        ...
        <div class="col-md-10 col-xs-8">
            <div class="vcard author post-author">
                ...
                <?php printf( esc_attr__('%1$s at %2$s', 'nrgbusiness'), get_comment_date(),  get_comment_time() ); ?>
            </div>
            <p class="comments-desc">
                <?php comment_text(); ?>
                <?php edit_comment_link( esc_attr__('Edit', 'nrgbusiness'), '  ', '' ); ?>           
